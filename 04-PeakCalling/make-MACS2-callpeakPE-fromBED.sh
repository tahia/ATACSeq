...
if [[ -z $1 ]] | [[ -z $2 ]] | [[ -z $3 ]] ; then
  echo "Usage: make-picard-sort.sh in/dir in/ctrl/path/to/bed out/dir "
  exit 1;
fi
...
for fil in ${INDIR}*.clean.TAG.bed; do
        BASE=$(basename $fil)
	NAME=${BASE%.clean.TAG.bed}
	OLOG="${LOG}${NAME}_callpeak.log"
   	echo "macs2 callpeak -t $fil -c $CTRL -f BED --keep-dup all -n $NAME --shift 73 --extsize 147 --max-gap 150 --fe-cutoff 1 \
